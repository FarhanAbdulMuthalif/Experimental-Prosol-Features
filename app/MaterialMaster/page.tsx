...
export default function MaterialMaster() {
  ...
  const FormCreationHandler = async (e: FormEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const DynamicFormSubmitHandler = async (e: FormEvent) => {
    ...
    // ValidateDynamic();
    const requiredStudentsWithScores = InputData.filter((student) => {
      return (
        student.required === true &&
        DynamicFieldData[student.fieldName]?.length > 0
      );
    });
    const reqdFilterAr = InputData?.filter((dta: any) => dta.required === true);
    console.log("+++++required is her +++++" + requiredStudentsWithScores);
    console.log(requiredStudentsWithScores);
    console.log(requiredStudentsWithScores.length);
    console.log(reqdFilterAr);
    if (
      Object.keys(DynamicFieldData).length === 0 ||
      requiredStudentsWithScores.length !== reqdFilterAr.length
    ) {
      console.log("inside.......");
      console.log(requiredStudentsWithScores);
      console.log(requiredStudentsWithScores.length);
  ...
  return (
    ...
    >
      ...
      >
        <form onSubmit={FormCreationHandler}>
